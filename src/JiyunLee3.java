// 과제 3번입니다. (double SHA256의 collision 찾기. starts with 0(1 to 20bit))
import java.security.MessageDigest;
import java.util.Arrays;

public class JiyunLee3 {

	public static void main(String[] args) {

	
		long start=System.currentTimeMillis(); // 경과 시간 측정 시작
		
		System.out.println("[SHA256 사용해서 데이터 인코딩(암호화)실시]");
		// 내가 구하고자 하는 0_bitsize를 입력합니다.
		int bitsize=1;
		int zeroCount=0;
		...
		while(loop) {
			 ...
			 */
			String data = "homework"+num; //초기 문자열 변수 선언 실시
			
			System.out.println("원본 : "+data);
			
			System.out.println("sha256 인코딩(문자열) : "+getSHA256(getSHA256(data)));
			
			byte encode[] = getSHA256_ByteArray(getSHA256(data));
			...
			if(zeroCount==bitsize) {
				System.out.println("찾았습니다. "+"homework"+num);
				loop=false;
			}
			else {
				zeroCount=0;
